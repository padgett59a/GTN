...
/****** Object:  StoredProcedure [dbo].[SP_GetTrxCourses]    Script Date: 2/8/2022 3:49:14 PM ******/
SET ANSI_NULLS ON
GO
...
IF OBJECT_ID(N'dbo.#tempTrxCourseTotals', N'U') IS NOT NULL  
   DROP TABLE [dbo].[#tempTrxCourseTotals];  
IF OBJECT_ID(N'dbo.#tempTrxCourseInWork', N'U') IS NOT NULL  
   DROP TABLE [dbo].[#tempTrxCourseInWork];  
...
CREATE CLUSTERED INDEX idx_temp_coursecoreid ON #tempTrxMrxStatus ([coursecoreid]);
CREATE NONCLUSTERED INDEX idx_temp_langid ON #tempTrxMrxStatus ([langid]);
...
CREATE CLUSTERED INDEX idx_tempTotals_coursecoreid ON #tempTrxCourseTotals ([coursecoreid]);
CREATE NONCLUSTERED INDEX idx_tempTotals_langid ON #tempTrxCourseTotals ([langid]);
...
CREATE CLUSTERED INDEX idx_tempInWork_coursecoreid ON #tempTrxCourseInWork ([coursecoreid]);
CREATE NONCLUSTERED INDEX idx_tempInWork_langid ON #tempTrxCourseInWork ([langid]);

-- Update status temp table with Trx sessions in work
update tS set tS.SessionTrxInWork = txS.SessionTrxInWork
from #tempTrxMrxStatus tS
join #tempTrxCourseInWork txS
  on (txS.courseCoreID = tS.courseCoreID
  and txs.langID = tS.langID); 
...
Drop table #tempTrxCourseInWork
Drop table #tempTrxCourseTotals
